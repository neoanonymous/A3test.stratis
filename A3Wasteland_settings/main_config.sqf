...
// Player settings
A3W_startingMoney = 10000000;           // Amount of money that players start with
...
A3W_remoteBombStoreRadius = 0;    // Prevent players from placing any kind of explosive on the ground within this distance from any store (0 = disabled)
...
A3W_vehiclePurchaseCooldown = 0;  // Number of seconds to wait before allowing someone to purchase another vehicle, don't bother setting it too high because it can be bypassed by rejoining
...
A3W_atmTransferFee = 1.5;            // Fee in percent charged to players for money transfers to other players (0 to 50)
