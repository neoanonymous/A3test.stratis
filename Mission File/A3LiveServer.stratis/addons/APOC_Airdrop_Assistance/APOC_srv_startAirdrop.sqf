...
_flyHeight = 75;  //Distance from ground that heli will fly at
...
//Delete heli once it has proceeded to end point
	[_heli,_grp,_flySpot,_dropSpot,_heliDistance] spawn {
		...
		while{([_heli, _flySpot] call BIS_fnc_distance2D)>80}do{
...
WaitUntil {((((position _object) select 2) < 1) || (isNil "_para"))};
		...
		if (_type == "vehicle2") then {_object allowDamage true;};
